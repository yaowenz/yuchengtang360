				...
				<tr>\
				  <td <% if (icon) { %>colspan="2"<% } %> class="aui_td_buttons">\
					  <div class="aui_buttons_wrap">\
						<% if (yesFn) { %><span class="aui_yes"><button id="<%=id%>yes" onfocus="this.blur()"><%=yesText%></button></span><% } %>\
						<% if (noFn) { %><span class="aui_no"><button id="<%=id%>no" onfocus="this.blur()"><%=noText%></button></span><% } %>\
					  </div>\
					</td>\
				</tr>\
				<% } %>\
