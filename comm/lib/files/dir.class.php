...
class dirs{
	 ...
	 * @return array
	 */
	static function get_dirname_for_cache($dirname){   
		$key = md5('get_dir_'.$dirname);
		$expire = 60*60;
		$cache = new temp_cache; 
		$g_info = $cache->get($key); 
		if (empty($g_info)){ 
			$g_info = self::get_dirname($dirname);
			$cache->saveOrUpdate($key,$g_info,$expire);
		} 
		$cache->close();
		if($g_info) return $g_info; else return false;
	}
	 ...
	 * @return array
	 */
	static function get_dirname($dirname){
		...
		while($file = readdir($handle)){
			...
			if(is_dir($newpath)){
				...
				$start = strrpos($newpath,'/');
				if($start === false){}else{$newpath = substr($newpath,$start+1);}
				if($newpath != '.' && $newpath != '..'){
					$ary[] = string::convert_encoding_deep($newpath);
				}
			}
		}
		return $ary;
