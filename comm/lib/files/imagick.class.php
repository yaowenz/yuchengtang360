	...
	*/
	...
	public function add_text($text, $x = 0 , $y = 0, $angle=0, $style=array())
	{
	    ...
	    if($this->type=='gif' && !$this->gif_off)
	    {
    	    ...
    	    {
    	        $draw->setFillColor('#454545');
				$this->image->annotateImage($draw, $x+1, $y+1, $angle, $text);
	...
	public function add_text2($text, $x = 0 , $y = 0, $angle=0, $style=array())
	{
        $draw = new ImagickDraw();
        if(isset($style['font'])) $draw->setFont($style['font']);
        if(isset($style['font_size'])) $draw->setFontSize($style['font_size']);
	    ...
	    if($this->type=='gif' && !$this->gif_off)
	    {
    	    ...
    	    {
				$draw->setFillColor('#454545');
				$this->image->annotateImage($draw, $x+3, $y-2, $angle, $text);
				if(isset($style['fill_color'])) $draw->setFillColor($style['fill_color']); 
				$this->image->annotateImage($draw, $x+2, $y-3, $angle, $text);
    	    }
	    } 
	    else
	    {
			//分析水印区域内颜色，如果很杂则添加背景，反之不添加背景
			$clra=null;
			for($xi=$x;$xi<$x+140;$xi++)
			{
				for($yi=$y-16;$yi<$y;$yi++)
				{
					$pixel = $this->image->getImagePixelColor($xi,$yi);
					$clr=$pixel->getColor();
					$clra[$clr['r'].$clr['g'].$clr['b'].$clr['a']]=1;
				}
			}
			if(count($clra)>1000)
			{
				$draw->setFillColor('#A1A1A1bb');
				//$draw->setStrokeColor('#ffffff40');
				//$draw->setStrokeWidth(0);
				//$draw->rectangle($x, $y+4,150,$y-16);
				$draw->roundRectangle($x+1,$y-17,$x+140,$y-2,7,7);
				$this->image->drawImage( $draw );
				if(isset($style['fill_color'])) $draw->setFillColor($style['fill_color']); 
				$draw->setStrokeColor('none');
				$this->image->annotateImage($draw, $x+7, $y-5, $angle, $text);
			}
			else
			{
				$draw->setFillColor('#454545');
				$this->image->annotateImage($draw, $x+3, $y-3, $angle, $text);
				if(isset($style['fill_color'])) $draw->setFillColor($style['fill_color']); 
				$draw->setStrokeColor('none');
				$this->image->annotateImage($draw, $x+2, $y-4, $angle, $text);
			}
	    }
	}
